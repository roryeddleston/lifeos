...
export default function HabitStreakBars({ data }: { data: Row[] }) {
  ...
  // Sort + compute max once per data change
  const { sorted, max } = useMemo(() => {
    const s = [...(data ?? [])].sort((a, b) => b.streak - a.streak);
    const m = Math.max(1, ...s.map((d) => d.streak));
      ...
      {isEmpty ? (
        ...
        >
          ...
          <p className="mt-1" style={{ color: "var(--twc-muted)" }}>
            Create a habit and mark it complete on consecutive days to see
            streaks here.
          </p>
        </div>
      ) : (
